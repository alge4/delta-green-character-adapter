...
- **Capability evidence** — `validateCapabilityEvidence` is the CI gate for exact directed capability records: inventory, known-loss, fixture checksums, and present artifacts must agree (#8).
- **Verified capability registry** — `parseVerifiedCapabilityRegistry` / `validateVerifiedCapabilityRegistry` advertise only the three exact initial directed capabilities from `docs/mappings/verified-capabilities.json` (#29). Adjacent versions cannot enter the registry.
